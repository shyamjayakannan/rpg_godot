...
public class DialogSystem : CanvasLayer
{
    ...
    {
        ShowDialog(dialogBranchResources[index].DialogItemResources);
        dialogBranchResources[index].QuestAdvanceResource?.AdvanceQuest();
        dialogBranchResources[index].EmitSignal(nameof(DialogBranch.Selected), dialogBranchResources[index].Text);
    }
    ...
    public void ShowDialog(List<DialogItemResource> items)
    {
        if (items.Count == 0)
        {
            SetUIState(false);
